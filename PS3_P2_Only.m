...
argp_depA  = deg2rad(-100.5043);
M_depA     = deg2rad(201.086 + 12.35936);
...
plot_all(...
  rYA_B/1e3, rLIN_B/1e3, rTR_B/1e3, ...
  vYA_B/1e3, vLIN_B/1e3, vTR_B/1e3, ...
...
plot_all(...
  rYA_C/1e3, rLIN_C/1e3, rTR_C/1e3, ...
  vYA_C/1e3, vLIN_C/1e3, vTR_C/1e3, ...
...
function [rYA, rLIN, rTR, vYA, vLIN, vTR, t_orbit] = run_case(...
  ...
  for k=1:N
    ...
    tau  = n*t/eta^3;
    Phi  = buildYAphi(a_chief,ecc_chief,f,nu0,tau);
    X    = Phi * Ks;
    rYA(k,:) = X(1:3)';
    vYA(k,:) = X(4:6)';
  end
...
function plot_all(rYA,rLIN,rTR, vYA,vLIN,vTR, t_orbit, titleStr)
  ...
  subplot(1,3,1)
    ...
    grid on; xlabel('T [km]'); ylabel('R [km]');
